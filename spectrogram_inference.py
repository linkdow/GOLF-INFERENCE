...
import sys
sys.path.append(r"/home/linkdow/svs/nnsvs/golf")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Inference script that takes spectrogram as input')
    parser.add_argument('--ckpt', default=None, help='Path to the checkpoint file')
    parser.add_argument('--input', default=None, help='Path to the input spectrogram file')
    parser.add_argument('--data_path', default=None, help='Path to the data folder if multiple spectrograms are to be processed')
    parser.add_argument('--config', help='Path to the config file')
    parser.add_argument('--sr', default=None, type=int, help='Sample rate of the input spectrogram')
    return parser.parse_args()


def main():

    # create a parser later if necessary for these variables :
    device = "cuda"
    lr = 0.001
    iterations = 1000
    ...
    if target_file is None and data_path is None:
        print("Please provide either --input or --data_path")
        return
    if target_file is not None:
        y, sr_target = torchaudio.load(target_file)
        y = y.to(device)
    else: # data_path is not None
        # process all files in the data_path
        pass
    ...
    assert sr == sr_target, "Sample rate of the input spectrogram does not match the provided sample rate"
